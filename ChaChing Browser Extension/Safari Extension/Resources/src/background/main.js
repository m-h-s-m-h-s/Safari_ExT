 ...
 * Loaded from excluded-domains.json for easier maintenance.
 * @type {string[]}
 */
let EXCLUDED_DOMAINS = [];

/**
 * Load excluded domains from the JSON file
 */
async function loadExcludedDomains() {
  try {
    const response = await fetch(chrome.runtime.getURL('src/data/excluded-domains.json'));
    const data = await response.json();
    EXCLUDED_DOMAINS = data.domains || [];
    console.log('[Background] Loaded', EXCLUDED_DOMAINS.length, 'excluded domains');
  } catch (error) {
    console.error('[Background] Failed to load excluded domains:', error);
    // Fallback to essential domains if file fails to load
    EXCLUDED_DOMAINS = [
      'chaching.me',
      'localhost',
      '127.0.0.1',
...
// Load excluded domains when service worker starts
loadExcludedDomains();
